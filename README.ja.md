...
**再起動**: Ctrl+C で停止し、`openwork` を再度実行します。
...
グローバルインストール後、`openwork` と `openwork` コマンドが使用できます：

| コマンド / オプション | 短縮形 | 説明 |
|------------------|-------|-------------|
| `openwork` または `openwork` | | サーバーを起動（デフォルト） |
| `openwork start` | | サーバーを明示的に起動 |
| `openwork status` | | 設定とデータの場所を表示 |
...
```bash
openwork                          # デフォルト設定で起動
openwork -p 8080              # カスタムポートで起動
openwork status                   # 現在の設定を表示
```

### バックグラウンドサービスとして実行（本番環境推奨）

本番環境では、PM2（Process Manager 2）を使用して OpenWork をバックグラウンドサービスとして実行します：
...
OpenWork はプロプライエタリソフトウェアです。利用と配布は組織内ポリシーに従ってください。
