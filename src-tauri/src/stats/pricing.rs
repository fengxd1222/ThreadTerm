...
pub const BUILTIN_PRICING_VERSION: &str = "builtin-cc-switch-2026-08";
...
pub fn lookup(model: &str) -> Option<ModelCosts> {
    ...
    if c.starts_with("claude-opus-4-5")
        || c.starts_with("claude-opus-4-6")
        || c.starts_with("claude-opus-4-7")
        || c.starts_with("claude-opus-4-8")
        || c.starts_with("claude-opus-5")
    {
        return Some(mtok(5.0, 25.0, 6.25, 0.5));
    }
    ...
    if c.starts_with("claude-haiku-4-5") {
        return Some(mtok(1.0, 5.0, 1.25, 0.1));
    }
    ...
    if c.starts_with("gpt-5.4") {
        return Some(mtok(2.5, 15.0, 0.0, 0.25));
    }
    if c.starts_with("gpt-5.2") || c.contains("codex") || c.starts_with("gpt-5") {
        return Some(mtok(1.75, 14.0, 0.0, 0.175));
    }
    ...
    if c.contains("gemini-2.5-flash-lite") {
        return Some(mtok(0.1, 0.4, 0.0, 0.01));
    }
    if c.contains("gemini-2.5-flash") || c.contains("gemini-flash") {
        return Some(mtok(0.3, 2.5, 0.0, 0.03));
    }
...
pub fn cost_breakdown(model: &str, usage: &UsageSummary) -> CostBreakdown {
    cost_breakdown_with_rates(lookup(model), usage)
}

/// Resolve a configured model override before falling back to the built-in
/// table. Keys are canonicalized so date pins and vendor prefixes behave like
/// cc-switch's model-pricing lookup.
...
pub fn cost_breakdown_with_rates(costs: Option<ModelCosts>, usage: &UsageSummary) -> CostBreakdown {
...
mod tests {
    ...
    fn canonical_strips_provider_and_pin() {
        assert_eq!(
            canonical_model("anthropic/claude-opus-4-8@20260101"),
            "claude-opus-4-8"
        );
    ...
    fn canonical_strips_iso_date_suffix() {
        assert_eq!(canonical_model("gpt-5.4-2026-03-05"), "gpt-5.4");
        assert_eq!(canonical_model("gpt-5.4-pro-2026-03-05"), "gpt-5.4-pro");
        assert_eq!(canonical_model("openai/gpt-5.4-2026-03-05"), "gpt-5.4");
    }

    #[test]
    fn canonical_strips_compact_date_suffix() {
        assert_eq!(canonical_model("gpt-5.4-20260305"), "gpt-5.4");
        assert_eq!(
            canonical_model("claude-opus-4-6-20260206"),
            "claude-opus-4-6"
        );
        assert_eq!(canonical_model("openai/gpt-5.4-20260305"), "gpt-5.4");
    }

    #[test]
    fn canonical_preserves_undated_names() {
        assert_eq!(canonical_model("gpt-5.4"), "gpt-5.4");
        assert_eq!(canonical_model("gpt-5.2-codex"), "gpt-5.2-codex");
        assert_eq!(canonical_model("o3"), "o3");
        assert_eq!(canonical_model("deepseek-chat"), "deepseek-chat");
    }
