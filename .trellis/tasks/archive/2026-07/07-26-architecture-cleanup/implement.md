...
- [x] `npm audit --omit=dev`：生产依赖 0 漏洞；剩余告警仅发布工具，
  需要跨大版本升级，保留确认。
- [x] 已完成全部大模块搬迁后的 detect-changes：34 个既定路径、277 个业务节点、
  74 条核心流程，因覆盖 Bridge、终端生命周期和工作台入口被评为 CRITICAL；
  文件范围、专项测试与全量门禁均与计划一致，未发现意外业务扩散。
