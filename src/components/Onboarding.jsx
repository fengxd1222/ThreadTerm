...
const Onboarding = ({ onComplete }) => {
  ...
  const handleNextStep = async () => {
    ...
    // Step 0: Git config validation and submission (optional)
    if (currentStep === 0) {
      // If both fields are empty, skip this step
      if (!gitName.trim() && !gitEmail.trim()) {
        setSkippedSteps(prev => new Set([...prev, 0]));
      ...
      if (!gitName.trim() || !gitEmail.trim()) {
        setError('Both git name and email are required, or leave both empty to skip');
        return;
      }
  ...
  const steps = [
    {
      ...
      description: 'Set up your git identity for commits (optional)',
      icon: GitBranch,
      required: false
    },
    {
      title: 'Connect Agents',
      description: 'Connect your AI coding assistants (optional)',
      icon: LogIn,
      required: false
    }
  ];
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="gitName" className="flex items-center gap-2 text-sm font-medium text-foreground mb-2">
                  ...
                  Git Name <span className="text-muted-foreground">(optional)</span>
                </label>
                <input
                  type="text"
                  id="gitName"
              ...
              <div>
                <label htmlFor="gitEmail" className="flex items-center gap-2 text-sm font-medium text-foreground mb-2">
                  ...
                  Git Email <span className="text-muted-foreground">(optional)</span>
                </label>
                <input
                  type="email"
                  id="gitEmail"
          ...
          <div className="space-y-6">
            ...
            <div className="text-center pt-4 border-t border-border">
              <button
                ...
                className="text-sm text-muted-foreground hover:text-foreground underline transition-colors"
              >
                Skip all and start using the app
              </button>
            </div>
          </div>
        );
  ...
  const isStepValid = () => {
    switch (currentStep) {
      case 0:
        // Git config is optional - valid if empty or both fields filled with valid email
        if (!gitName.trim() && !gitEmail.trim()) {
          return true; // Both empty is valid (will skip)
        }
          ...
          <div className="bg-card rounded-lg shadow-lg border border-border p-8">
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
              <div className="flex items-center gap-3">
                ...
                {/* Skip All button - visible on all steps */}
                <button
                  onClick={handleSkipAll}
                  disabled={isSubmitting}
                  className="text-sm text-muted-foreground hover:text-foreground underline transition-colors"
