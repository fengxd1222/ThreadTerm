...
export default function MainContentStateView({ mode, projectName }: MainContentStateViewProps) {
  const { t } = useTranslation();

  const isLoading = mode === 'loading';
  const isProjectLanding = mode === 'project';
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md mx-auto px-6">
            ...
            <h2 className="text-xl font-semibold mb-2 text-foreground">
              {isProjectLanding ? t('mainContent.projectReadyTitle') : t('mainContent.chooseProject')}
            </h2>
            <p className="text-sm text-muted-foreground mb-5 leading-relaxed">
              {isProjectLanding
                ? t('mainContent.projectReadyDescription', { project: projectName || '' })
