...
function CredentialsSettings() {
  ...
  // Version check hook
  const { updateAvailable, latestVersion, releaseInfo } = useVersionCheck('openwork', 'openwork');
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch source hosting credentials only
  ...
  const createGithubCredential = async () => {
    ...
    try {
      ...
      console.error('Error creating source hosting credential:', error);
    }
  };
  ...
  const deleteGithubCredential = async (credentialId) => {
    ...
    try {
      ...
      console.error('Error deleting source hosting credential:', error);
    }
  };
  ...
  const toggleGithubCredential = async (credentialId, isActive) => {
    try {
      ...
      console.error('Error toggling source hosting credential:', error);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* source hosting Credentials Section */}
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
        ...
        {showNewGithubForm && (
          <div className="mb-4 p-4 border rounded-lg bg-card space-y-3">
            ...
            <a
              href="https://source.example.com/settings/tokens"
              target="_blank"
              rel="noopener noreferrer"
      ...
      <div className="pt-6 border-t border-border/50">
        <div className="flex items-center justify-between text-xs italic text-muted-foreground/60">
          <a
            href={releaseInfo?.htmlUrl || 'https://source.example.com/openwork/openwork/releases'}
            target="_blank"
            rel="noopener noreferrer"
