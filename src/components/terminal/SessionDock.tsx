    ...
    <section
      ...
      className={[
        ...
        variant === 'panel'
          ...
          : 'absolute bottom-0 right-0 top-0 z-[35] w-64 border-l border-border',
        visible
          ...
          <div className="space-y-1">
            {recentCards.map((card, index) => (
              <SessionDockRow
                ...
                card={card}
                index={index}
                isCurrent={card.id === focusedCardId}
...
const SessionDockRow = memo(function SessionDockRow({
  card,
  index,
  isCurrent,
  isHighlighted,
    ...
    >
      <div className="flex min-w-0 items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-1.5">
          {index < 9 && (
            <span className="flex h-4 min-w-4 shrink-0 items-center justify-center rounded bg-muted px-1 font-mono text-[11px] text-muted-foreground">
              {index + 1}
            </span>
          )}
          ...
          {chip && (
            <span className="max-w-[45%] shrink-0 truncate rounded bg-muted px-1 py-0.5 text-[11px] font-medium text-muted-foreground">
              {chip}
            </span>
          )}
        </div>
...
}, areSessionDockRowPropsEqual);

function areSessionDockRowPropsEqual(
  prev: SessionDockRowProps,
  next: SessionDockRowProps,
): boolean {
  return (
    prev.card === next.card &&
