 ...
 * OpenWork CLI
 *
 * Provides command-line utilities for managing OpenWork
 *
 * Commands:
...
function showStatus() {
    ...
    console.log('\n' + c.dim('═'.repeat(60)));
    console.log(`\n${c.tip('[TIP]')} Hints:`);
    console.log(`      ${c.dim('>')} Use ${c.bright('openwork --port 8080')} to run on a custom port`);
    console.log(`      ${c.dim('>')} Use ${c.bright('openwork --database-path /path/to/db')} for custom database`);
    console.log(`      ${c.dim('>')} Run ${c.bright('openwork help')} for all options`);
...
Commands:
  start          Start the OpenWork server (default)
  status         Show configuration and data locations
...
Examples:
  ...
  $ openwork --port 8080            # Start on port 8080
  $ openwork -p 3000                # Short form for port
  $ openwork start --port 4000      # Explicit start command
  $ openwork status                 # Show configuration
...
Documentation:
  ${packageJson.homepage || 'https://source.example.com/openwork/openwork'}

Report Issues:
  ${packageJson.bugs?.url || 'https://source.example.com/openwork/openwork/issues'}
`);
}
...
async function checkForUpdates(silent = false) {
    try {
        ...
        if (isNewerVersion(latestVersion, currentVersion)) {
            ...
            console.log(`         Run ${c.bright('openwork update')} to update\n`);
...
async function updatePackage() {
    try {
        ...
        console.log(`${c.tip('[TIP]')} Try running manually: npm update -g @openwork/openwork`);
    }
}
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('   Run "openwork help" for usage information.\n');
            process.exit(1);
    }
}
