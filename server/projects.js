 ...
 * ========================================
 *
 * This module manages project discovery for Claude and Codex sessions.
 */
...
async function isGitRepository(projectPath) {
  ...
  if (projectGitRepoCache.has(normalizedPath)) {
    return projectGitRepoCache.get(normalizedPath);
  }

  try {
    await fs.access(projectPath);
  } catch {
    projectGitRepoCache.set(normalizedPath, false);
    return false;
  }

  try {
    await fs.access(path.join(projectPath, '.git'));
    projectGitRepoCache.set(normalizedPath, true);
    return true;
  } catch {}

  try {
    const { stdout } = await runGitCommand(['rev-parse', '--is-inside-work-tree'], projectPath);
    const isRepo = stdout.trim() === 'true';
    projectGitRepoCache.set(normalizedPath, isRepo);
    return isRepo;
  } catch {
    projectGitRepoCache.set(normalizedPath, false);
    return false;
...
async function getProjects(progressCallback = null) {
  ...
  try {
    ...
    for (const entry of directories) {
        ...
        // Get display name and workspace metadata from config
        ...
        const project = {
          ...
          isGitRepo: false,
          workspaceType: projectConfig.workspaceType || null,
          isGitWorktree: projectConfig.isGitWorktree || projectConfig.workspaceType === 'git-worktree',
          branch: projectConfig.branch || null,
          sourceProjectName: projectConfig.sourceProjectName || null,
          repoRoot: projectConfig.repoRoot || null,
          worktreePath: projectConfig.worktreePath || null,
          worktreeBaseRoot: projectConfig.worktreeBaseRoot || null,
          sessions: [],
          sessionMeta: {
            hasMore: false,
            total: 0
        ...
        try {
          project.isGitRepo = await isGitRepository(actualProjectDir);
        } catch {
...
async function renameProject(projectName, newDisplayName) {
  ...
  const trimmedDisplayName = typeof newDisplayName === 'string' ? newDisplayName.trim() : '';

  if (!trimmedDisplayName) {
    // Remove only the custom display name. Keep metadata (manuallyAdded/originalPath/etc)
    // so manual workspaces do not disappear from the project list.
    const { displayName, ...restConfig } = existingConfig;
    if (Object.keys(restConfig).length === 0) {
      delete config[projectName];
    } else {
      config[projectName] = restConfig;
    }
  } else {
    // Preserve existing metadata and only update display name.
    config[projectName] = {
      ...existingConfig,
...
async function addProjectManually(projectPath, displayName = null) {
  ...
  if (config[projectName]) {
    const existingConfig = config[projectName] || {};
    const hasManualMetadata = Boolean(existingConfig.manuallyAdded || existingConfig.originalPath);
    ...
    // Recover entries broken by legacy rename logic that overwrote manual metadata.
    // If there is no backing project directory and the config has no manual metadata,
    // repair the entry in-place instead of blocking with "already configured".
    if (!hasProjectDirectory && !hasManualMetadata) {
      config[projectName] = {
        ...existingConfig,
        manuallyAdded: true,
      ...
      return {
        ...
        isManuallyAdded: true,
        isGitRepo: await isGitRepository(absolutePath),
        sessions: [],
        codexSessions: []
      };
    }
  ...
  for (const filePath of jsonlFiles) {
    try {
      ...
      const session = {
        ...
        sandboxType: sessionData.sandboxType,
        permissionModeHint: sessionData.permissionModeHint,
      };
...
function mapCodexPermissionModeHint(approvalPolicy, sandboxType) {
  const normalizedApprovalPolicy = typeof approvalPolicy === 'string'
    ? approvalPolicy.trim().toLowerCase()
    : '';
  const normalizedSandboxType = typeof sandboxType === 'string'
    ? sandboxType.trim().toLowerCase()
    : '';

  if (normalizedApprovalPolicy === 'never' || normalizedSandboxType === 'danger-full-access') {
    return 'bypassPermissions';
  }

  if (normalizedApprovalPolicy || normalizedSandboxType) {
    return 'acceptEdits';
  }
  ...
  try {
      ...
      if (line.trim()) {
        try {
          ...
          if (entry.type === 'turn_context' && entry.payload) {
            if (typeof entry.payload.approval_policy === 'string') {
              approvalPolicy = entry.payload.approval_policy;
            }
            if (typeof entry.payload.sandbox_policy?.type === 'string') {
              sandboxType = entry.payload.sandbox_policy.type;
            }
          }
    ...
    if (sessionMeta) {
      return {
        ...
        approvalPolicy,
        sandboxType,
        permissionModeHint: mapCodexPermissionModeHint(approvalPolicy, sandboxType),
      };
    }
